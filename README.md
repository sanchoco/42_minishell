...
쉘의 존재 이유는 IT에 있어 밀접하게 연결되어 있습니다. 컴퓨터와 통신하기 위해 0과 1을 스위치를 통해 나열하는건 짜증나는 행동이였습니다. 논리적으로 컴퓨터와 소통을 하기 위한 아이디어로 영어에 가까운 대화형 커맨드를 생각하게 되었습니다. 

Minishell 프로젝트와 함께, Windows가 존재하지 않았을 때 당시로 여행하여 문제점에 직면할 수 있습니다.

### 지침

- Norm에 준수하여야 하며 에러가 뜬다면 0점을 받을 것 입니다.
- 함수가 예기치 않게 종료되지 않아야 합니다.
- 할당된 메모리는 반드시 해제해야 하고 누수가 발생하지 않아야 합니다.
- 이 과제는 반드시 Makefile로 제출되어야 하며 컴파일할 때, -Wall -Wextra -Werror 플래그를 사용해야 하고, relink가 되어선 안됩니다.
- Makefile은 $(NAME), all, clean, fclean, re 규칙이 있어야 합니다.
- 보너스를 할 때, Makefile에 bonus 규칙이 있어야 하며 필수 파트와 별개로 _bonus.{c/h}파일이 있어야 합니다.
- libft 프로젝트를 사용할 수 있으며, libft폴더 안에 Makefile이 있어야 합니다. Makefile은 라이브러리를 Makefile로 컴파일할 수 있어야 합니다.
- 깃 레포지토리에 당신의 작업을 제출할 때, 오직 레포지토리 안에서만 평가됩니다. Deepthought로 평가될 때 오류가 발생한다면 평가는 중지됩니다.

### 필수 파트

- 프로그램 이름: minishell
- Makefile: 필요
- 허용 함수: malloc, free, write, open, read, close, fork, wait, waitpid, wait3, wait4, signal, kill, exit, getcwd, chdir, stat, lstat, fstat, execve, dup, dup2, pipe, opendir, readdir, closedir, strerror, errno
- Libft 허용: Yes
- 기술: 쉘을 작성하세요.
...
- 새 명령을 기다릴 때 프롬프트 표시
- bash와 같이 올바르게 실행 파일을 찾고 실행합니다.(환경 변수 또는 상대, 절대경로를 기반)
- bash와 같이 반드시 존재해야 하는 것.
    1. echo와 -n 옵션
    ...
    6. env 옵션 x 인자x
    7. exit 옵션 x
    8. ; bash와 같이 명령을 분리할 수 있어야 함.
    9. ' 와 " bash와 같이 여러 명령줄을 받을 수 있어야 함.
    10. 리다이렉션 < > ">>"은 file descriptor를 제외하고 bash와 같이 동작
    11. 파이프 | bash와 같이 동작
    12. 환경 변수 ($ 뒤 문자) bash와 같이 동작
    13. $? bash와 같이 동작
    14. ctrl-C, ctrl-D, ctrl-\ 의 결과가 bash와 같아야 함.
